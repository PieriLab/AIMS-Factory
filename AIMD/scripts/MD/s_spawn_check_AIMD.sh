...
RECENT_MIN=5
# how many lines from the end of md.out to check for success or error
TAIL_N=250
# regex for terachem errors
...
scan aims/<run>/aimd/<spawn>/md.out and aims/<run>/aimd/<spawn>/r*/md.out
report: done | running | error | not started yet

writes failing base md.out paths to:   $ERROR_BASE
writes failing restart md.out paths to: $ERROR_RESTART
...
error_in_md_out() {
  ...
  local tailtxt
  tailtxt="$(tail -n "$TAIL_N" -- "$f" || true)"
  grep -qiE "$ERR_RE" <<<"$tailtxt" && return 0
  return 1
}
...
print_restart_info() {
  local d="$1"
  echo "    [files in $d]"
  local required=(coords.xyz vels.xyz md.in submit.sh)
...
for run_dir in "${runs[@]}"; do
  ...
  for spawn_dir in "$aimd_dir"/*; do
    ...
    # check base md.out if present
    if [[ -e "$spawn_dir/md.out" ]]; then
      md="$spawn_dir/md.out"
      label="$run_id: spawn $spawn_id"
      err_file="$ERROR_BASE"

      if success_in_md_out "$md"; then
        echo "$label: done"
      elif error_in_md_out "$md"; then
        echo "$label: error ($md)"
        echo "$md" >> "$err_file"
      elif recent_activity "$spawn_dir"; then
        echo "$label: running"
      else
        echo "$label: error (no termination flag and stale) ($md)"
        echo "$md" >> "$err_file"
      fi
    fi
